...
class CSP {
public:
    std::vector<V> variables;  // Variables to be assigned
    std::unordered_map<V, std::vector<D>> domains;  // Domain of each variable
    std::unordered_map<V, std::vector<std::shared_ptr<Constraint<V, D>>>> constraints;  // Constraints on variables
     ...
     */
    CSP(const std::vector<V>& variables, const std::unordered_map<V, std::vector<D>>& domains) : variables(variables), domains(domains) {
        for (const auto& variable : variables) {
            constraints[variable] = std::vector<std::shared_ptr<Constraint<V, D>>>();
    ...
    bool consistent(V variable, std::unordered_map<V, D>& assignment) {
        for (const auto& constraint : constraints[variable]) {
